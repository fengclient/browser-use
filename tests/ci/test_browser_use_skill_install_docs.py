...
def test_docs_install_browser_use_skill_from_package_alias():
	...
	assert 'run `browser-use-volcengine skill install` to register the skill' in readme
...
def test_browser_use_cli_installs_browser_harness_package_skill(tmp_path):
	...
	assert uv_args.read_text(encoding='utf-8') == 'tool install --python 3.12 --upgrade --force browser-use-volcengine'
	expected = (
		'---\n'
		'name: browser-use\n'
