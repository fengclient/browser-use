...
def test_browser_use_doctor_help_prints_browser_use_usage():
	...
	assert result.stdout == 'usage: browser-use-volcengine doctor [--fix-snap]\n'
...
def test_normalize_captured_cli_output_handles_string_system_exit(capsys):
	...
	assert captured.out == ''
	assert captured.err == 'browser-use-volcengine failed\n'
...
def test_browser_use_tui_is_deprecated_alias(monkeypatch, capsys):
	...
	assert capsys.readouterr().err == 'browser-use-volcengine-tui is deprecated; use browser-use-volcengine instead.\n'
