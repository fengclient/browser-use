...
CLI_COMMAND = 'browser-use-volcengine'
TUI_COMMAND = 'browser-use-volcengine-tui'


def _browser_use_version() -> str:
	for package_name in ('browser-use-volcengine', 'browser-use'):
		try:
...
def _set_harness_client_env() -> None:
	...
	os.environ['BH_CLIENT'] = 'browser-use-volcengine-cli'
...
def _run_install_command(argv: list[str]) -> int:
	if any(arg in {'-h', '--help'} for arg in argv):
		print(f'usage: {CLI_COMMAND} install')
		print()
	...
	if result.returncode == 0:
		...
		print(f'Ready to use. Run: uvx {CLI_COMMAND}')
		return 0
...
def _as_browser_use_cli_text(text: str) -> str:
	return text.replace('Browser Harness', 'Browser Use').replace('browser-harness', CLI_COMMAND)
...
def _run_browser_harness() -> int | None:
	...
	if args and args[0] == 'doctor' and args[1:]:
		if args[1:] in (['--help'], ['-h']):
			print(f'usage: {CLI_COMMAND} doctor [--fix-snap]')
			return 0
		if args[1:] != ['--fix-snap']:
			print(f'usage: {CLI_COMMAND} doctor [--fix-snap]', file=sys.stderr)
			sys.exit(2)
...
_LEGACY_HINTS: dict[str, str] = {
	...
	'close': f'# restart the local daemon with `{CLI_COMMAND} --reload`; stop cloud browsers with stop_remote_daemon(name)',
	...
	'cloud': f'# authenticate with `{CLI_COMMAND} auth login`, then start_remote_daemon("<name>")',
	'daemon': f'# the daemon starts automatically on every call; restart it with `{CLI_COMMAND} --reload`',
	'record': '# session recording was removed; use capture_screenshot() per step',
	'mcp': f'# MCP server mode is now the --mcp flag: `{CLI_COMMAND} --mcp`',
	...
	'-c': f"# pipe code on stdin instead: echo 'print(page_info())' | {CLI_COMMAND}",
	'--code': f"# pipe code on stdin instead: echo 'print(page_info())' | {CLI_COMMAND}",
}
...
Read the full interface now:   browser-use-volcengine skill show
Install the CLI 3.0 skill (upgrades the CLI and replaces any pre-existing browser-use skill):
                               browser-use-volcengine skill install
...
Health check:                  browser-use-volcengine --doctor"""
...
def _legacy_migration_message(command: str) -> str:
	...
	if hint.startswith('#'):
		...
		replacement = f"  {CLI_COMMAND} <<'PY'\n  {hint}\n  PY"
	return f"""The {CLI_COMMAND} CLI changed in 3.0, and '{command}' was removed.
...
def _unknown_helper_message(name: str) -> str:
	return f"""'{name}' is not defined in the {CLI_COMMAND} CLI.

Example:
  browser-use-volcengine <<'PY'
  new_tab("https://example.com")
  print(page_info())
  PY
...
You can also paste this into your agent to get started:
  ...
  Install or upgrade browser-use-volcengine to the latest stable version with uv using
  Python 3.12, register the skill from `browser-use-volcengine skill`, and connect it to
  my browser. Follow https://github.com/browser-use/browser-use if setup or
  connection fails.

More:
  browser-use-volcengine --doctor     check install, daemon, and browser health
  browser-use-volcengine --help       full command list
  docs: https://github.com/browser-use/browser-use/blob/main/browser_use/skills/browser-use/SKILL.md"""

_EMPTY_STDIN_MESSAGE = """browser-use-volcengine received empty stdin. This CLI executes Python piped on stdin:
  browser-use-volcengine <<'PY'
  print(page_info())
  PY"""
...
def browser_use_tui_main() -> int | None:
	print(f'{TUI_COMMAND} is deprecated; use {CLI_COMMAND} instead.', file=sys.stderr)
	return main()
