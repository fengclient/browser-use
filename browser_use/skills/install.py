...
CLI_NAME = 'browser-use-volcengine'
DEFAULT_TARGET = 'all'
...
def _install_browser_use_tool() -> None:
	uv = shutil.which('uv')
	if not uv:
		raise RuntimeError(f'Installing the Browser Use skill requires `uv`. Install uv, then rerun `{CLI_NAME} skill install`.')
...
def _build_parser() -> argparse.ArgumentParser:
	...
	install.add_argument(
		'--no-install',
		action='store_true',
		help=f'Skip uv tool install/upgrade and use the existing {CLI_NAME} command or package',
	)
