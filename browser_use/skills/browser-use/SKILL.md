...
- Invoke as `browser-use-volcengine`. Use heredocs for multi-line commands.
...
Ask the user to tick "Allow remote debugging for this browser instance" and click Allow if Chrome shows a permission popup. Then retry the same `browser-use-volcengine` command.
...
```bash
browser-use-volcengine auth login
```

Or import a key safely:

```bash
printf '%s' "$BROWSER_USE_API_KEY" | browser-use-volcengine auth login --api-key-stdin
```
...
BU_NAME=r7k2 browser-use-volcengine <<'PY'
...
```bash
browser-use-volcengine recordings enable
browser-use-volcengine recordings disable
browser-use-volcengine recordings
```
...
```bash
browser-use-volcengine recordings --latest
```
